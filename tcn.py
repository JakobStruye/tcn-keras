...
def add_temporal_block(inputs, n_inputs, n_outputs, kernel_size, stride, dilation, padding, dropout=0.2):
    conv1 = Conv1D(n_outputs, kernel_size, strides=stride, dilation_rate=dilation, padding='same' )(inputs)
    #weightnorm
    #chomp?
    relu1= Activation('relu')(conv1)
    dropout1 = Dropout(dropout)(relu1)
    conv2 = Conv1D(n_outputs, kernel_size, strides=stride, dilation_rate=dilation, padding='same' )(dropout1)
    #chomp?
...
if __name__ == '__main__':
    ...
    train_X, train_Y = build_data(50000,seq_len)
    test_X, test_Y = build_data(1000, seq_len)
